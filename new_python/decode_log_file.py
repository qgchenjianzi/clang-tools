...
NEW_MAGIC_CRYPT_START = 0x03;
NEW_MAGIC_COMPRESS_CRYPT_START = 0x04;
...
def IsGoodLogBuffer(_buffer, _offset, count):
	...
	if MAGIC_CRYPT_START==_buffer[_offset] or MAGIC_COMPRESS_CRYPT_START==_buffer[_offset]:
		headerLen = 1 + 4
	elif NEW_MAGIC_CRYPT_START==_buffer[_offset] or NEW_MAGIC_COMPRESS_CRYPT_START==_buffer[_offset]:
		headerLen = 1 + 2 + 1 + 1 + 4
	else:
...
def GetLogStartPos(_buffer, _count):
	...
	while True:
		...
		if MAGIC_CRYPT_START==_buffer[offset] or MAGIC_COMPRESS_CRYPT_START==_buffer[offset] or NEW_MAGIC_CRYPT_START==_buffer[offset] or NEW_MAGIC_COMPRESS_CRYPT_START==_buffer[offset]:
...
def DecodeBuffer(_buffer, _offset, _outbuffer):
	...
	if MAGIC_COMPRESS_CRYPT_START==_buffer[_offset] or MAGIC_CRYPT_START==_buffer[_offset]:
		key = BASE_KEY ^ (0xff & length) ^ _buffer[_offset]

	else:
		seq=struct.unpack_from("H", buffer(_buffer, _offset+headerLen-4-2-2, 2))[0]
		begin_hour=struct.unpack_from("c", buffer(_buffer, _offset+headerLen-4-1-1, 1))[0]
		end_hour=struct.unpack_from("c", buffer(_buffer, _offset+headerLen-4-1, 1))[0]
		key = BASE_KEY ^ (0xff & seq) ^ _buffer[_offset]
	...
	for i in range(length):
		tmpbuffer[i] = key ^ _buffer[_offset+headerLen+i]

	try:
		
		if MAGIC_COMPRESS_CRYPT_START==_buffer[_offset]:
			tmpbuffer = zlib.decompress(str(tmpbuffer))
		elif NEW_MAGIC_COMPRESS_CRYPT_START==_buffer[_offset]:
